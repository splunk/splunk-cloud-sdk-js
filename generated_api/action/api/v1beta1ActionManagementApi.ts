...
export class V1beta1ActionManagementApi {
     ...
     * @param tenant Tenant name/identifier
     */
    public getActions (authorization: string, tenant: string, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.ClientResponse; body: Array<DeprecatedAction>;  }> {
        const localVarPath = this.basePath + '/{tenant}/action/v1beta1/actions'
            .replace('{' + 'tenant' + '}', encodeURIComponent(String(tenant)));
        let localVarQueryParameters: any = {};
        let localVarHeaderParams: any = (<any>Object).assign({}, this.defaultHeaders);
        ...
        return new Promise<{ response: http.ClientResponse; body: Array<DeprecatedAction>;  }>((resolve, reject) => {
            localVarRequest(localVarRequestOptions, (error, response, body) => {
                if (error) {
                    reject(error);
                } else {
