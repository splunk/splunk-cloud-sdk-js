...
export abstract class SearchServiceGen extends BaseApiService {
     ...
     * @return SearchJob
     */
    public createJob = (searchJob?: SearchJob, queryParams?: object): Promise<SearchJob> => {
        if (!searchJob) {
            throw new SplunkError({ message: `Bad Request: searchJob is empty or undefined` });
        }
        const path = `/search/v2beta1/jobs`;
        return this.client.post(SEARCH_SERVICE_CLUSTER, this.client.buildPath('', path.split('/').slice(1)), searchJob, { query: queryParams })
     ...
     * @return SearchJob
     */
    public getJob = (sid: string, queryParams?: object): Promise<SearchJob> => {
        const path_params = {
            sid: sid
        };
        const path = this.template`/search/v2beta1/jobs/${'sid'}`(path_params);
        return this.client.get(SEARCH_SERVICE_CLUSTER, this.client.buildPath('', path.split('/').slice(1)), { query: queryParams })
            .then(response => response.body as SearchJob);
    }
    /**
     ...
     * @param sid The search ID.
     * @param queryParams Query parameters to be sent with the request
     * @param queryParams.count The maximum number of entries to return. Set to 0 to return all available entries. 
     * @param queryParams.earliest The earliest time filter, in absolute time. When specifying an absolute time specify either UNIX time, or UTC in seconds using the ISO-8601 (%FT%T.%Q) format.  For example 2019-01-25T13:15:30Z. GMT is the default timezone. You must specify GMT when you specify UTC. Any offset specified is ignored. 
     * @param queryParams.field A field to return for the result set. You can specify multiple fields of comma-separated values if multiple fields are required. 
     * @param queryParams.latest The latest time filter in absolute time. When specifying an absolute time specify either UNIX time, or UTC in seconds using the ISO-8601 (%FT%T.%Q) format.  For example 2019-01-25T13:15:30Z. GMT is the default timezone. You must specify GMT when you specify UTC. Any offset specified is ignored. 
     * @param queryParams.offset Index of first item to return.
     * @return ListSearchResultsResponse
     */
    public listEventsSummary = (sid: string, queryParams?: { count?: number, earliest?: string, field?: string, latest?: string, offset?: number, [key: string]: any }): Promise<ListSearchResultsResponse> => {
        const path_params = {
            sid: sid
        };
     ...
     */
    public listFieldsSummary = (sid: string, queryParams?: { earliest?: string, latest?: string, [key: string]: any }): Promise<FieldsSummary> => {
        const path_params = {
            sid: sid
        };
     ...
     * @param queryParams Query parameters to be sent with the request
     * @param queryParams.count The maximum number of jobs that you want to return the status entries for. 
     * @param queryParams.status Filter the list of jobs by status. Valid status values are 'running', 'done', 'canceled', or 'failed'. 
     * @return Array<SearchJob>
     */
    public listJobs = (queryParams?: { count?: number, status?: SearchStatus, [key: string]: any }): Promise<Array<SearchJob>> => {
        const path = `/search/v2beta1/jobs`;
        return this.client.get(SEARCH_SERVICE_CLUSTER, this.client.buildPath('', path.split('/').slice(1)), { query: queryParams })
     ...
     */
    public listPreviewResults = (sid: string, queryParams?: { count?: number, offset?: number, [key: string]: any }): Promise<ListPreviewResultsResponse> => {
        const path_params = {
            sid: sid
        };
     ...
     * @return ListSearchResultsResponse
     */
    public listResults = (sid: string, queryParams?: { count?: number, field?: string, offset?: number, [key: string]: any }): Promise<ListSearchResultsResponse> => {
        const path_params = {
            sid: sid
        };
     ...
     */
    public listTimeBuckets = (sid: string, queryParams?: object): Promise<TimeBucketsSummary> => {
        const path_params = {
            sid: sid
        };
     ...
     * @return SearchJob
     */
    public updateJob = (sid: string, updateJob?: UpdateJob, queryParams?: object): Promise<SearchJob> => {
        if (!updateJob) {
        ...
        const path = this.template`/search/v2beta1/jobs/${'sid'}`(path_params);
        return this.client.patch(SEARCH_SERVICE_CLUSTER, this.client.buildPath('', path.split('/').slice(1)), updateJob, { query: queryParams })
            .then(response => response.body as SearchJob);
    }
}
