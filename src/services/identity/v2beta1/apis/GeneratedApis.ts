...
export class GeneratedIdentityService extends BaseApiService {
     ...
     * @return Array<string>
     */
    public listMemberPermissions = (member: string, args?: { scopeFilter?: string, [key: string]: any }): Promise<Array<string>> => {
        const path_params = {
            member: member
        };
     ...
     * @param args parameters to be sent with the request
     */
    public setPrincipalPublicKeys = (principal: string, args?: object): Promise<object> => {
        const path_params = {
            principal: principal
        };
        const path = this.template`/system/identity/v2beta1/principals/${'principal'}/keys`(path_params);
        return this.client.put(IDENTITY_SERVICE_CLUSTER, this.client.buildPath('', path.split('/').slice(1)), { query: args })
            .then(response => response.body as object);
    }
    /**
